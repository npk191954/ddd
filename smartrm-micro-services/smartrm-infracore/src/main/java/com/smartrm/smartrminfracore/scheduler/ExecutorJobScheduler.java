package com.smartrm.smartrminfracore.scheduler;
...
public class ExecutorJobScheduler {
    ...
    public void schedule(Class<? extends ExecutorBase> jobClass, Map<String, Object> params, int delay) {
        Date date = new Date(System.currentTimeMillis() + delay);
        SimpleTrigger trigger = (SimpleTrigger) newTrigger().startAt(futureDate(delay, IntervalUnit.MILLISECOND))
                .build();
     ...
     */
    public void scheduleRetry(Class<? extends RetryExecutorBase> jobClass, Map<String, Object> params, int delay,
            int retryInterval) {
    ...
    public void scheduleRetry(Class<? extends RetryExecutorBase> jobClass, Map<String, Object> params, int delay,
        ...
        Date date = new Date(System.currentTimeMillis() + delay);
        SimpleTrigger trigger = (SimpleTrigger) newTrigger().startAt(futureDate(delay, IntervalUnit.MILLISECOND))
                .withSchedule(simpleSchedule().withIntervalInMilliseconds(retryInterval).withRepeatCount(retryTimes))
                .build();
        JobDataMap dataMap = new JobDataMap(params);
        JobDetail job = JobBuilder.newJob(jobClass).setJobData(dataMap).build();
        try {
