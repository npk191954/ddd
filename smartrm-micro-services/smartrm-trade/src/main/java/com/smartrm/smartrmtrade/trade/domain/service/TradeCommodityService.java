...
public interface TradeCommodityService {
    ...
    default List<StockedCommodity> inventoryToCommodity(List<InventoryInfo> inventoryInfos) {
        ...
        List<CommodityInfo> infos = getCommodityList(inventoryInfos.stream().map(info -> {
            inventoryMap.put(info.getCommodityId(), info.getCount());
            return info.getCommodityId();
        }).collect(Collectors.toList()));
        return infos.stream()
                .map(info -> new StockedCommodity(info.getCommodityId(), info.getCommodityName(), info.getImageUrl(),
                        info.getPrice(), inventoryMap.get(info.getCommodityId()))).collect(Collectors.toList());
