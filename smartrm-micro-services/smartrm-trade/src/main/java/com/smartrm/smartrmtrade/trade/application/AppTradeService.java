...
public class AppTradeService {
    ...
    public VendingMachineCommodityListDto queryCommodityList(long machineId) {
        ...
        return new VendingMachineCommodityListDto(machine.getCommodityList(deviceService, commodityService),
                machine.getState());
    }
    ...
    public PaymentQrCode selectCommodity(SelectCommodityCmdDto cmd) {
        ...
        StockedCommodity commodity = new StockedCommodity(commodityInfo.getCommodityId(),
                commodityInfo.getCommodityName(), commodityInfo.getImageUrl(), commodityInfo.getPrice(), 1);
        PaymentQrCode code = machine
                .selectCommodity(Lists.newArrayList(commodity), deviceService, payService, cmd.getPlatformType());
    ...
    public void onCabinetLocked(CabinetVendingMachineLockedEvent event) {
        ...
        try {
            ...
            payService.chargeForOrder(OrderInfo.Builder().machineId(order.getMachineId()).orderId(order.getOrderId())
                    .type(order.getType()).totalAmount(order.totalAmount()).build(), userInfo);
