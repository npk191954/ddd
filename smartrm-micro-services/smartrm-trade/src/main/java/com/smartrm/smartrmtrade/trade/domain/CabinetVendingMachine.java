...
public class CabinetVendingMachine {
    ...
    public Order onLocked(CabinetVendingMachineLockedEvent event, TradeCommodityService commodityService) {
        ...
        event.getInventorySnapshotWhenOpen().forEach(info -> preInventory.put(info.getCommodityId(), info.getCount()));
        List<StockedCommodity> commodities = commodityService
                .inventoryToCommodity(event.getInventoryWhenLock().stream().flatMap(info -> {
    ...
    private Order generateOrder(Collection<StockedCommodity> commodities) {
        Order order = Order.Builder().commodities(commodities).orderId(UniqueIdGeneratorUtil.instance().nextId())
                .state(OrderState.Start).type(OrderType.CabinetAutoDeduction).machineId(this.machineId)
